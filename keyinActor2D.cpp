...
vtkStandardNewMacro(keyinActor2D);

vtkCxxSetObjectMacro(keyinActor2D, LabelTextProperty, vtkTextProperty);
vtkCxxSetObjectMacro(keyinActor2D, TitleTextProperty, vtkTextProperty);

//----------------------------------------------------------------------------
// Instantiate this object.
keyinActor2D::keyinActor2D()
{
	...
	this->UseFontSizeFromProperty = 1;
	...
	this->pointActor1 = vtkActor2D::New();
	this->pointActor1->SetMapper(pointMapper);
	this->pointActor2 = vtkActor2D::New();
	this->pointActor2->SetMapper(pointMapper);
}
...
int keyinActor2D::RenderOpaqueGeometry(vtkViewport *viewport)
{
...
int keyinActor2D::RenderOverlay(vtkViewport *viewport)
{
	int i, renderedSomething = 0;
...
vtkTypeBool keyinActor2D::HasTranslucentPolygonalGeometry()
{
	return 0;
}
...
void keyinActor2D::ReleaseGraphicsResources(vtkWindow *win)
{
...
//----------------------------------------------------------------------------
void keyinActor2D::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void keyinActor2D::BuildAxis(vtkViewport *viewport)
{
	...
	if (!this->LabelVisibility)
	{
		...
		for (i = 0; i < this->AdjustedNumberOfLabels; i++)
		{
			...
			this->SetOffsetPosition(xTick,theta,this->LastMaxLabelSize[0],this->LastMaxLabelSize[1],this->TickOffset,this->LabelActors[i]);
	...
	if (this->Title != nullptr && this->Title[0] != 0 && this->TitleVisibility)
	{
		...
		//this->SetOffsetPosition(xTick,theta,stringSize[0],stringSize[1],static_cast<int>(offset),this->TitleActor);
		this->TitleActor->SetPosition(p1[0] + (this->imageSize[0]) / 2 + pointSize[0] / 2 + this->labelOffset[0], p1[1] + this->labelOffset[1]);

		this->imageActor->SetPosition(p1[0] + pointSize[0] / 2 + this->labelOffset[0], p1[1] - this->imageSize[1] / 2 + this->labelOffset[1]);
...
//----------------------------------------------------------------------------
void keyinActor2D::UpdateAdjustedRange()
{
...
//reasonable interval
void keyinActor2D::ComputeRange(double inRange[2],double outRange[2],int vtkNotUsed(inNumTicks),int &numTicks,double &interval)
{
	// Handle the range
	double sRange[2];
...
void keyinActor2D::SetOffsetPosition(double xTick[3], double theta,int stringWidth, int stringHeight,int offset, vtkActor2D *actor)
{
	double x, y, center[2];
	int pos[2];
...
//----------------------------------------------------------------------------
double keyinActor2D::ComputeStringOffset(double width, double height,double theta)
{
...
//----------------------------------------------------------------------------
void keyinActor2D::ShallowCopy(vtkProp *prop)
{
	keyinActor2D *a = keyinActor2D::SafeDownCast(prop);
	if (a != nullptr)
	{
...
void keyinActor2D::SetTextBackImageFile(const char * textImageFile)
{
...
void keyinActor2D::LabelOffset(double * off)
{
	this->labelOffset[0] += off[0];
	this->labelOffset[1] += off[1];
}
