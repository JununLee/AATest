...
vtkStandardNewMacro(keyinAngleActor);

vtkCxxSetObjectMacro(keyinAngleActor, LabelTextProperty, vtkTextProperty);


//----------------------------------------------------------------------------
// Instantiate this object.
keyinAngleActor::keyinAngleActor()
{
	...
	this->ArrowPlacement = keyinAngleActor::VTK_ARROW_BOTH;
	this->ArrowStyle = keyinAngleActor::VTK_ARROW_FILLED;
	...
	imageActor = vtkActor2D::New();
	imageActor->SetMapper(imageMapper);

	
	pointMapper = vtkImageMapper::New();
	pointMapper->SetColorWindow(256);
	pointMapper->SetColorLevel(128);
	pointActor1 = vtkActor2D::New();
	pointActor1->SetMapper(pointMapper);
	pointActor2 = vtkActor2D::New();
	pointActor2->SetMapper(pointMapper);
...
keyinAngleActor::~keyinAngleActor()
{
	...
	this->pointActor1->Delete();
	this->pointActor2->Delete();
...
//----------------------------------------------------------------------------
void keyinAngleActor::BuildLeader(vtkViewport *viewport)
{
	// Check to see whether we need to rebuild-----------------------------
	int positionsHaveChanged = 0;
	if (viewport->GetMTime() > this->BuildTime || (viewport->GetVTKWindow() && viewport->GetVTKWindow()->GetMTime() > this->BuildTime))
	{
		...
		if (lastPosition[0] != this->LastPosition[0] || lastPosition[1] != this->LastPosition[1] || lastPosition2[0] != this->LastPosition2[0] || lastPosition2[1] != this->LastPosition2[1])
		{
	...
	this->pointActor1->SetPosition(p1[0] - pointSize[0] / 2, p1[1] - pointSize[1] / 2);
	this->pointActor2->SetPosition(p2[0] - pointSize[0] / 2, p2[1] - pointSize[1] / 2);
	...
	if (this->AutoLabel || (this->Label != nullptr && this->Label[0] != 0))
	{
		...
		if ((clippedLeader = this->ClipLeader(xL, stringSize, p1, ray, c1, c2)))
		{
			...
			this->LabelActor->SetPosition(p1[0], p1[1]);
	...
	// Build the arrows---------------------------------------
	if (this->ArrowPlacement == keyinAngleActor::VTK_ARROW_NONE)
	{
		; //do nothin
	}
	else //we are creating arrows
	{
		...
		if (this->ArrowPlacement == keyinAngleActor::VTK_ARROW_POINT1 ||
			this->ArrowPlacement == keyinAngleActor::VTK_ARROW_BOTH)
		{
			...
			if (this->ArrowStyle == keyinAngleActor::VTK_ARROW_FILLED)
			{
			...
			else if (this->ArrowStyle == keyinAngleActor::VTK_ARROW_OPEN)
			{
			...
			else //if ( this->ArrowStyle == keyinAngleActor::VTK_ARROW_HOLLOW )
			{
		...
		if (this->ArrowPlacement == keyinAngleActor::VTK_ARROW_POINT2 ||
			this->ArrowPlacement == keyinAngleActor::VTK_ARROW_BOTH)
		{
...
int keyinAngleActor::SetFontSize(vtkViewport *viewport, vtkTextMapper *textMapper,int *targetSize, double factor, int *stringSize)
{
...
//----------------------------------------------------------------------------
int keyinAngleActor::ClipLeader(double center[3], int box[2], double p1[3],double ray[3], double c1[3], double c2[3])
{
...
//----------------------------------------------------------------------------
void keyinAngleActor::BuildCurvedLeader(double p1[3], double p2[3], double ray[3],double rayLength, double theta,vtkViewport *viewport, int viewportChanged)
{
	...
	//std::cout<<"ARC::  " << p2[0] << " " << p2[1] << std::endl;
	//this->LabelActor->SetPosition(center[0] + this->imageSize[0] / 2 + this->pointSize[0] / 2, center[1]);
	//this->imageActor->SetPosition(center[0] + this->pointSize[0] / 2, center[1] - this->imageSize[1] / 2);
	this->LabelActor->SetPosition(centerPos[0] + this->imageSize[0] / 2 + this->pointSize[0] / 2, centerPos[1]);
	this->imageActor->SetPosition(centerPos[0] + this->pointSize[0] / 2, centerPos[1] - this->imageSize[1] / 2);
...
int keyinAngleActor::InStringBox(double center[3], int stringSize[2], double x[3])
{
...
bool keyinAngleActor::MouseOverLabel(double * p)
{
	double* labelPos = imageActor->GetPosition();
	double xmin = labelPos[0];
	double xmax = labelPos[0] + imageSize[0];
	double ymin = labelPos[1];
	double ymax = labelPos[1] + imageSize[1];
...
void keyinAngleActor::ReleaseGraphicsResources(vtkWindow *win)
{
...
int keyinAngleActor::RenderOpaqueGeometry(vtkViewport *viewport)
{
	...
	renderedSomething += this->pointActor1->RenderOpaqueGeometry(viewport);
	renderedSomething += this->pointActor2->RenderOpaqueGeometry(viewport);
	// Everything is built, just have to render
	if ((this->Label != nullptr && this->Label[0]) || \
		(this->AutoLabel && this->LabelMapper->GetInput() != nullptr))
	{

		renderedSomething += this->imageActor->RenderOpaqueGeometry(viewport);
...
int keyinAngleActor::RenderOverlay(vtkViewport *viewport)
{
	int renderedSomething = 0;
	this->BuildLeader(viewport);
	...
	{
		renderedSomething += this->pointActor1->RenderOverlay(viewport);
		renderedSomething += this->pointActor2->RenderOverlay(viewport);
	}
	return renderedSomething;
}
...
vtkTypeBool keyinAngleActor::HasTranslucentPolygonalGeometry()
{
	return 0;
}

//----------------------------------------------------------------------------
void keyinAngleActor::ShallowCopy(vtkProp *prop)
{
	keyinAngleActor *a = keyinAngleActor::SafeDownCast(prop);
	if (a != nullptr)
	{
...
void keyinAngleActor::SetTextBackImageFile(const char * textImageFile)
{
...
void keyinAngleActor::SetPointImageFile(const char * pointImageFile)
{
	vtkSmartPointer<vtkPNGReader> pointImage = vtkSmartPointer<vtkPNGReader>::New();
	pointImage->SetFileName(pointImageFile);
	pointImage->Update();
	pointImage->GetOutput()->GetDimensions(pointSize);
	pointMapper->SetInputData(pointImage->GetOutput());
}
//----------------------------------------------------------------------------
void keyinAngleActor::PrintSelf(ostream& os, vtkIndent indent)
{
